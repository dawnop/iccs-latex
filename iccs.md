...
第二章讨论了MPL的定义。根据MPL的定义，筛选出IEEE Spectrum上的具有MPL特征的流行编程语言。并给出了良好的编程语言设计应当考虑的因素。第三章站在了编程范式的角度。给出了判断MPL编程范式支持程度的方法，并对被选择的MPLs支持程度做了分析。之后探讨了编程范式支持强弱和编程语言应用之间关系。第四章讨论了类型系统。根据L Cardelli的类型系统定义，讨论了被选择的MPLs的类型系统。第五章通过三个基准测试，分别从内存分配、浮点运算、IO，讨论了编程语言的性能。然后，站在编程语言实现的角度，结合这些语言的应用场景，分析基准测试的数据，解释了这些MPLs性能表现的原因。
